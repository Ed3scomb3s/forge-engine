"""RL Training V5 — "The Swing Trader"
======================================
Fundamental paradigm shift from V1-V4:

WHY PREVIOUS VERSIONS FAILED:
  - 1h candles = 24 decisions/day = overtrading
  - Agent trades every candle = massive fee drag
  - MACD baseline wins with 5 trades in 7 months

V5 PHILOSOPHY: "Trade less, trade better"
  1. 4h candles — 6 decisions/day instead of 24 (75% fewer)
  2. MACD + ATR in observations — give agent the signals we KNOW work
  3. Very wide dead zone (0.45) — only trade when very confident
  4. Tiny network [32, 32] — can't memorize, must generalize
  5. Tight SL (2.5%), wide TP (10%) — 4:1 reward/risk ratio
  6. Data from 2020 — maximum training diversity
  7. 4M steps — plenty of training for the simpler 4h environment
  8. Sortino reward — proven to work, penalizes downside only

Usage:
    uv run python evaluation/train_rl_v5.py
    uv run python evaluation/train_rl_v5.py --quick   (1 fold, 1 seed — fast test)
"""
...
SYMBOL = "BTCUSDT_PERP"
TIMEFRAME = "4h"                 # V5: 4-hour candles (was 1h)
TF_MINUTES = 240                 # 4 * 60
WARMUP_CANDLES = 50
...
PPO_V5_CONFIG = dict(
    observations=[
        ...
        "returns",
        "rsi_14",
        "atr_14",                # Volatility — agent can learn "only trade when vol is right"
        "sma_ratio_20_50",       # Long-term trend
        "sma_ratio_10_30",       # Medium-term trend
        "ema_21",                # Fast trend
        "position_info",
        "drawdown",
    ],
    actions_factory=lambda: ContinuousActions(
        max_margin_pct=0.12,
        threshold=0.45,          # V5: VERY wide dead zone — only strong signals pass
        sl_pct=0.025,            # Tight SL (2.5%) — cut losers fast
        tp_pct=0.10,             # Wide TP (10%) — 4:1 reward/risk ratio
    ),
    reward="sortino",
    reward_kwargs=dict(eta=0.01, min_std=0.003),
    engine_kwargs=dict(
        ...
        max_steps=180,           # 180 4h-candles = 30 days per episode
    ),
    ...
    algo_factory=lambda env, seed: PPO(
        ...
        policy_kwargs=dict(
            net_arch=dict(pi=[32, 32], vf=[32, 32]),  # TINY — forces generalization
        ),
    ),
    algo_name="PPO_v5",
)
...
class DataCache:
    ...
    def load(cls):
        if cls._loaded:
            return
        t0 = time.time()
        print("Loading 4h candle data from 2020...")
        tmp = create_session(
        ...
        print(f"  Loaded {n:,} 4h-candles in {time.time() - t0:.1f}s\n")


def make_env(start_iso, end_iso, max_steps_override=None):
    DataCache.load()
    config = PPO_V5_CONFIG
...
def run_training(seeds=SEEDS, quick=False):
    config = PPO_V5_CONFIG
    algo_name = config["algo_name"]
    train_steps = 500_000 if quick else config["train_steps"]
    n_splits = 1 if quick else N_SPLITS
    seeds_to_use = seeds[:1] if quick else seeds

    print("=" * 70)
    print(f"  {algo_name} {'QUICK TEST' if quick else 'WFA Training'} -- {SYMBOL} (4h candles)")
    print("=" * 70)
    ...
    print(f"  Folds: {len(splits)}, Seeds: {len(seeds_to_use)}, Steps/fold: {train_steps:,}")
    print(f"  Timeframe: 4h (6 candles/day) — swing trader mode")
    print(f"  Dead zone: 0.45 — extreme selectivity")
    print(f"  SL/TP: 2.5%/10% (4:1 ratio)")
    print(f"  Network: [32, 32] — forced simplicity")
    if holdout:
    ...
    results = {
        "algorithm": algo_name, "symbol": SYMBOL, "version": "v5",
        "config": {
            "observations": config["observations"],
            ...
            "wfa_mode": WFA_MODE, "seeds": list(seeds_to_use),
    ...
    for fold_idx, (tr_start, tr_end, te_start, te_end) in enumerate(splits):
        ...
        for seed_idx, seed in enumerate(seeds_to_use):
            t0 = time.time()
            print(f"    Seed {seed_idx+1}/{len(seeds_to_use)} (seed={seed})... ", end="", flush=True)
            try:
...
def main():
    parser = argparse.ArgumentParser(description="RL V5 — The Swing Trader")
    parser.add_argument("--quick", action="store_true", help="Quick test: 1 fold, 1 seed")
    parser.add_argument("--seeds", type=int, default=3)
    args = parser.parse_args()
    seeds = SEEDS[:args.seeds]
    run_training(seeds=seeds, quick=args.quick)
